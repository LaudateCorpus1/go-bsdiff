...
func TestDiff(t *testing.T) {
	...
	z := []byte{
		...
		0x6d, 0xe9, 0x73, 0x54, 0x8f, 0x9f, 0xac, 0x05, 0x68, 0x1e, 0xdb, 0xcc, 0xa3, 0xbb, 0x02, 0xf9,
		0x90, 0x65, 0x27, 0x1a, 0x16, 0x3b, 0x14, 0x14, 0x4d, 0x13, 0xdb, 0x23, 0x1c, 0xd5, 0x09, 0xa4,
	...
	if !bytes.Equal(diffbs[:len(z)], z) {
		diff := byteDiff(z, diffbs[:len(z)])

		t.Fatal("diff is non-zero\n" + hex.Dump(diff))
	}
}

// assume the lengths are equal
func byteDiff(expected, actual []byte) []byte {
	diff := make([]byte, 0, len(expected))
	for i := range expected {
		diff = append(diff, actual[i]-expected[i])
	}
	return diff
}
